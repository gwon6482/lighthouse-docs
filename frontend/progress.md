...
가입 진로답변 서버 전송(`42d433c`)이 v0.1.8 뒤 커밋이라, 9/17 까지 프로덕션에서는
저장 경로가 끊겨 있었다**(v0.1.9 로 해소). 다행히 그 사이 신규 가입이 없어 실제 유실은 0건이었다.
...
- **R5 — 인증 경계**: 로그인 페이지(`/onboarding/auth`)에 **로그인 상태로** 닿으면 그 자리에 머물지 않는다.
  판정은 `shared/router/authBoundary.ts`(순수 함수, 회귀 테스트 5건), 실행은 `shared/router/app.ts` beforeEach.

  | 상황 | 동작 |
  |---|---|
  | 토큰 없음 | 통과 (정상적인 로그인 화면) |
  | 토큰 있음 + 표시 없음 | **앱 안(`/`)으로 되돌림.** 로그아웃하지 않는다 |
  | 토큰 있음 + `?logout=1` | 확인 후 로그아웃 (의도적 이탈) |

  > ⚠️ **2026-09-17 개정.** 원래 규칙은 "토큰 있으면 **무조건** 로그아웃 confirm"이었다.
  > R5 를 세운 2026-07-22 에는 그 자리에 닿는 길이 '의도적 뒤로가기'뿐이라 맞는 전제였는데,
  > 카카오 로그인이 그 전제를 깼다 —
  > `/onboarding/auth → (카카오) → /onboarding/oauth → 위저드 → /onboarding/intro`
  > 로그인 화면이 **히스토리 바로 뒤**에 남아, 가입 직후 뒤로가기 한 번에 실제로 로그아웃됐다.
  >
  > 되돌릴 수 없는 쪽(로그아웃)을 기본값으로 두지 않는다 — 잘못 되돌리면 사용자는 앱에 남을 뿐이지만
  > 잘못 로그아웃하면 복구가 없다. 의도 표시(`?logout=1`)는 위저드 첫 화면의 '이전'이 붙여 준다.
  >
  > 부수 효과: bounce 뒤 루트 가드가 상태를 보고 보내므로 **위저드를 안 끝낸 소셜 계정은
  > 위저드로 자동 복귀**한다.
