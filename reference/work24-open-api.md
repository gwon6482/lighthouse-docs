...
공통은 **성취·경제적 보상 2개뿐**이다. 워크넷이 가치관 체계를 개편한 것으로 보인다.

⚠️ **이게 추천 알고리즘을 직접 깬다.** `recommendController.js` 에서
**T23 가치관 매칭이 추천 점수의 20%** 이고, `T23_1 → VA10` 식으로 우리 13개 코드(VA01~VA13)에
1:1로 묶여 있다. API 의 9개 체계로 갈아타면 이 매핑이 통째로 무효가 된다.
